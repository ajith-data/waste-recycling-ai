...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (password.length < 6) {
      toast({ title: t("register.passwordMin"), variant: "destructive" });
      return;
    }
    setLoading(true);
    ...
    if (error) {
      ...
      toast({ title: t("register.success"), description: t("register.canSignIn") });
      navigate("/login");
    }
  };
      ...
      <div className="w-full max-w-md bg-card rounded-2xl border border-border shadow-xl p-8 animate-fade-in">
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <Button type="submit" disabled={loading} className="w-full eco-gradient text-primary-foreground">
            {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : t("register.create")}
          </Button>
        </form>
